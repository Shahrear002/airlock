...
async fn upload_recursive(
    ...
    if metadata.is_dir() {
        ...
        while let Some(entry) = entries.next_entry().await.map_err(|e| e.to_string())? {
            ...
            Box::pin(upload_recursive(sftp, &entry_path, &remote_entry_path)).await?;
        }
    } else {
    ...
    match sftp.read_dir(&remote_path_str).await {
        Ok(dir) => {
            ...
            for entry in dir {
                ...
                Box::pin(download_recursive(sftp, &remote_entry_path, &local_entry_path)).await?;
            }
        }
        Err(_) => {
...
pub async fn sftp_upload(
    ...
    upload_recursive(&sftp, Path::new(&local_path), Path::new(&remote_path)).await?;
    Ok(())
}
...
pub async fn sftp_download(
    ...
    download_recursive(&sftp, Path::new(&remote_path), Path::new(&local_path)).await?;
    Ok(())
}
